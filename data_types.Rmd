...
One small different between the `L` suffix and the `as.integer()` function though, is that the former will not convert values that would lose information as an integer to integer, it will keep it as a numeric. `as.integer()` will always return an integer however:
