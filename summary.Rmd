...
Why do they evaluate to different things?
        + The first is a single expression. When we evaluate the first, both lines `x <- 1` and `x + 10` are evaluated at (roughly) the same time. The second is a list of separate expressions. If we evaluate them using
```{r}
lapply(
    list(
      quote(x <- 1),
...
```
Then each line is evaluated separetly, resulting in two lines instead of 1. 
