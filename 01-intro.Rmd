...
Technically speaking, even single values are stored as a vector in R, they just have length one.
...
More specifically, R stores dataframes as essentially a collection of lists, with each list representing a different column. To demonstrate this, when we type...
...
P.S. If you have a vector of named values, you can also use the names instead of the indices.

But Adam, I hear you ask, `c(10,20,30,40)[1]` and `c(10,20,30,40)[[1]]` just gave us the same thing, so are the interchangable?

Well, they kind of returned the same thing, but they didn't. So no, they're not interchangeable. Essentially, `[]` returns the *container* at the provided index, where `[[]]` returns the *value* at the provided index. Let's see a practical example of how these are different:
...
Matrices are two dimension, meaning they can't be subsetting with a single value. Instead, we still use the `[]` operator, but we provide two values, one for the row and another for the column:
...
Dataframes can be subsetted in the same way as matrices (using the `[]` operator). However, dataframes can also be subsetting (a bit like lists), using the `$` operator and the name of the column:
